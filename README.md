...
using "Fix all" can be enabled by setting `unsafe-fixes = true` in your Ruff configuration file.
...
The extension supports Jupyter Notebooks via the [Notebook Document Synchronization] capabilities
introduced in version 3.17 of the Language Server Protocol, including diagnostics, code actions,
and formatting.
...
notebooks are now linted and formatted by default. Earlier versions require explicitly including notebooks
with the [`extend-include`](https://docs.astral.sh/ruff/settings/#extend-include) setting in the Ruff
configuration file:
...
_Note: The above-mentioned Notebook configurations run the action for each cell individually.
This is how VS Code handles these Notebook actions and is unrelated to the Ruff extension. To run them on
the entire notebook at once, use the `Ruff` prefixed commands such as
...
You can configure some common settings in VS Code directly, like `ruff.lineLength` (the line length for the linter
and formatter) or `ruff.lint.select` (the enabled lint rules):
...
Refer to the [migration guide](https://docs.astral.sh/ruff/editors/migration/) for more details.
...
The status bar item displays the status of the Ruff language server.
Click it to open the Ruff output channel.
