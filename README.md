...
Reload VS Code after installing either extension to enable Python environment detection.
...
Ruff's automatic fixes are labeled as "safe" and "unsafe". By default, the "Fix all" action will not apply unsafe fixes. However, unsafe fixes can be applied manually with the "Quick fix" action. To enable unsafe fixes when using "Fix all", set `unsafe-fixes = true` in your Ruff configuration file.
...
The extension supports Jupyter Notebooks via the [Notebook Document Synchronization] capabilities introduced in version 3.17 of the Language Server Protocol, including diagnostics, code actions, and formatting.

Jupyter Notebook support was stabilized in [Ruff `0.6.0`](https://astral.sh/blog/ruff-v0.6.0#jupyter-notebooks-are-now-linted-and-formatted-by-default), and notebooks are now linted and formatted by default. Earlier versions require explicitly including notebooks with the [`extend-include`](https://docs.astral.sh/ruff/settings/#extend-include) setting in the Ruff configuration file:
...
The extension supports loading in an [untrusted workspace](https://code.visualstudio.com/docs/editor/workspace-trust). In an untrusted workspace, it always uses the bundled Ruff executable, regardless of any executable or Python environment settings.
...
_Note: The above-mentioned Notebook configurations run the action for each cell individually. This is how VS Code handles these Notebook actions and is unrelated to the Ruff extension. To run them on the entire notebook at once, use the `Ruff` prefixed commands such as `Ruff: Organize Imports` and `Ruff: Fix all auto-fixable problems`._
...
You can configure some common settings in VS Code directly, like `ruff.lineLength` (the line length for the linter and formatter) or `ruff.lint.select` (the enabled lint rules):
...
The extension now always uses the Rust-based language server (`ruff server`). The deprecated Python-based server (`ruff-lsp`) is no longer included. The `ruff.nativeServer` setting is deprecated and has no effect, including when set to `off` or `false`; remove it from your settings.

The legacy settings `ruff.args`, `ruff.lint.args`, `ruff.format.args`, `ruff.run`, `ruff.lint.run`, `ruff.ignoreStandardLibrary`, and `ruff.showNotifications` are no longer supported. Move any command-line configuration to `pyproject.toml` or `ruff.toml`, or use the supported editor settings. See the [migration guide](https://docs.astral.sh/ruff/editors/migration/) for details and settings without direct replacements.

For example, replace `ruff.lint.args: ["--extend-select=UP"]` with:

```json
{
  "ruff.lint.extendSelect": ["UP"]
}
```

Use `ruff.configuration` to select a custom configuration file instead of passing `--config` through `ruff.lint.args` or `ruff.format.args`.
...
| Ruff: Print debug information       | Print debug information about the language server |
...
The status bar item displays the status of the Ruff language server. Click it to open the Ruff output channel.
